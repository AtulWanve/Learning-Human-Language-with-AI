...
def ai_word_search(request):
    """API Endpoint: Retrieve word data generated by AI (auth + rate-limited)"""
    word = request.GET.get('word', '').strip()
    ...
    if not user_id:
        return JsonResponse(
            {"error": "Login required to use AI lookup."},
            status=401,
        )
    ...
    # Per-user rate limit: 3 AI lookups per 60-second window
    if is_rate_limited(f"ai_lookup:user:{user_id}", limit=3, window_seconds=60):
        return JsonResponse(
            {"error": "You've used AI lookup too many times. Please wait a minute."},
            status=429,
        )
...
def get_daily_content(request):
    """API Endpoint: Daily content (public, IP-rate-limited)"""
    logger.info("Received request for daily content")

    # Per-IP rate limit: 10 requests per 60-second window (generous for page reloads)
    client_ip = get_client_ip(request)
    if is_rate_limited(f"daily:ip:{client_ip}", limit=10, window_seconds=60):
        return JsonResponse(
